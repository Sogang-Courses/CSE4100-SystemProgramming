...
/* prj1 신지원) 테스트를 위한 expand 함수 구현 */
void bitmap_expand (struct bitmap *b, int size){
  ASSERT(b != NULL);

  size_t old_cnt = b->bit_cnt;
  b->bit_cnt += (size_t)size;
  b->bits = realloc(b->bits, byte_cnt (b->bit_cnt + (size_t)size));
  bitmap_set_multiple(b, old_cnt, (size_t)size, false);
}
